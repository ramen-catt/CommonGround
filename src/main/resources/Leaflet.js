...
//icon for normal common ground locations
...
//places them all to load on the map
function renderCommonGrounds() {
    dynamicLayer.clearLayers();
    for (const cg of commonGrounds) {
        const marker = L.marker([cg.lat, cg.lon], { icon: commonGroundIcon }).addTo(dynamicLayer);
        marker.bindPopup(`<b>${cg.name}</b><br>${cg.address}`);
    }
}
//API KEY SUPER IMPORTANT FOR GEOCODING
const EsriKey = "AAPTadWWGuAAgVxflKQV3KUYdfw..ldf0mZlOZA3zmhAwX8LLjTnxTKRws0R0IBedC81BXLgWxkkORzfHX5AIeVu3rG1dy3y5INkOwiX8bZ_EZI1ageKEQSC7FdpGZfB0jO5Jv2yRKIrwzhxjXm9sI_-H9gzyQHT6rdxDeXhTlUBepgHzm1rmznYQfzdgJlhjT3ZU85enL9ONzGG8ICdXj9dXiz2yTdNU3lA5m2DdvG0Jop5vFsvW_5uTgbbLhodKpIzD1bER7BB1LnY.AT1_VTOxrQ0l";

function geocodeAddress(address, callback) {
    L.esri.Geocoding.geocode({
        apikey: EsriKey }).text(address).run(function (err, results) {
...
//allows the location for the selected commonground to have visual feedback
...
//fetches closest midpoint and highlights them
            fetch(url)
                .then(res => res.text())
                .then(closestHPDName => {
